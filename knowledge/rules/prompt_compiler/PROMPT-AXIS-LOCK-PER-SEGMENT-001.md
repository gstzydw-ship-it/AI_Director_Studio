...
output_contract: "每个 segment 使用同侧轴线表达：同侧过肩、同侧固定、同侧正面微侧或场景侧面固定机位。"
example_good: "商北琛半身中景，同侧正面微侧机位；乔熙中近景，同侧过肩机位。"
...
Seedance 2.0 是单镜头连续生成模型。大模型尤其不稳定的一点是：它分不清“人物左侧”和“画面左侧”，而人物从正面转到背面后，人物自己的左侧会变成画面另一侧。一个 segment 里只要使用“商北琛左前方、乔熙右前方、人物左后方”这类人物相对左右机位，模型就容易把空间翻面、人物左右颠倒、背景错乱。

## 规则

每个 prompt segment 必须保持同侧轴线。最终 prompt 优先使用“同侧过肩机位、同侧固定机位、同侧正面微侧机位、办公桌侧面固定机位、门口侧面固定机位、背后跟拍”这类简洁表达。

**硬约束：**

1. 同一 segment 的【时间轴】**禁止**使用人物相对的"左前方、右前方、左后方、右后方"作为机位。
2. 同一 segment **禁止**出现"反打至 / 反打镜头 / 翻拍至"等剪辑性反打词。真要反打必须**拆段**，不能在单段 prompt 内做。
3. 镜头句里尽量不使用“左侧 / 右侧”；必须交代关系时，改写成“桌前/桌后”“同侧过肩”“同侧固定”“门口侧面”等可拍表达。
4. 主体转身或换向时，不沿用人物左右；改用新的同侧机位或拆成下一片段。
...
- **固定机位接管**：从人物相对机位切换到门口侧面、办公桌侧面或同侧固定机位作为中转。

## 禁止项

- 禁止写"反打至 X" 在单段 prompt 里。
- 禁止人物相对左右机位：`商北琛左前方`、`乔熙右前方`、`人物左后方`、`主体右后方`。
...
3-6秒：
镜头切至严飞中近景，同侧固定机位。严飞迈出半步，朝商北琛说出"Welcome, Mr. Pierce."后停住。
```
...
- `shot_director` 在镜头方案里就要锁好同侧轴线和简洁机位，不能让 compiler 自由发挥人物左右。
- `prompt_compiler` 输出前用 regex 自检：单 segment 内若出现人物相对左右机位，立即报硬失败。
