...
- DOC-STORY-SPLIT-005
conflict_resolution: "15秒是单片段硬上限，不是必须凑满的长度。普通段仍优先 13-15 秒；命中高密度快切、钩子阻断、动作压入或切镜预算超载时，允许 4-10 秒短段，但必须写 short_segment_reason 与 script_basis；禁止建议任何超过15秒的单片段。"
supersedes: []
---
...
editing_tempo_directive:
  segment_duration_policy:
    default: "13-15秒仍是普通导演片段容量，15秒是单片段硬上限"
...
15 秒是单片段硬上限，不是必须凑满的机械尺子。节奏总控不得向结构规划师输出 16 秒以上或 `18-22秒` 这类单片段建议；当事件密度超过 15 秒容量时，只能压缩无信息过渡、拆成相邻片段，或明确给出短段理由。

普通段：

- 普通对白、关系推进、动作承接仍优先 13-15 秒，且不得超过 15 秒。
- 能在同一动作段内完成的受击、反应、收束，不要为了快而拆碎。
- 如果完整发言、信息揭示和必要反应无法在 15 秒内清楚承载，必须重新划分为相邻片段，不能把单段拉长到 16 秒以上。
...
- 转场标识、字幕标识、闪回开始/闪回结束是硬边界。照片触发、人物识别、孩子台词和现实第一反应应先作为现实触发段处理；闪回本体从闪回开始标识之后独立承接，不得为了节奏紧凑把触发反应和闪回混成一个超过 15 秒的段。
...
- 禁止输出半截字段、半截句子或缺失 `风险提醒`；内容过长时减少条目数量，不能截断字段。
