...
def _story_planner_repair_prompt(
    *,
    ...
    return (
        ...
        "10. 节奏信息优先写进合同字段（duration_target、event_atom、emotion_delta、reaction_handoff、tail_state_required、shot_director_handoff）；节奏类型、事件密度判断、片段内节奏分配、动作节奏指导作为辅助字段可选填。\n"
        ...
        "13. 合并相邻片段时不要丢失节奏差异；把差异压进 event_atom、emotion_delta、reaction_handoff 和 shot_director_handoff，可补充片段内节奏分配作为辅助。\n"
        ...
        "14A. 合同字段用固定英文键；节奏辅助字段（节奏类型、事件密度判断、片段内节奏分配、动作节奏指导、出现人物、入场状态、出场状态）排在合同字段之后选填。禁止用旧版“片段任务/承接要求/镜头导演交接”替代合同字段。event_atom 只写一个可见事件原子。\n"
...
def _rhythm_story_planner_user_prompt(
    *,
    ...
    return (
        ...
        "每个片段的字段分为【合同字段】（必填，校验）和【节奏辅助字段】（选填，给 shot_director 参考）两层：\n"
        ...
        "- 合同字段用固定英文键，节奏辅助字段排在其后选填。禁止用旧版“片段任务/承接要求/镜头导演交接”替代合同字段。\n\n"
        "【分段与节奏规则】\n"
