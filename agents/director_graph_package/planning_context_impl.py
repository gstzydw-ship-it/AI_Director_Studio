...
def _run_director_showrunner_logic_review(
    *,
    ...
    system_prompt = (
        ...
        "4. 制片可拍性审查员：检查演员能否执行、动作密度、是否镜头化、是否过度微动作；简单主动作不得被拆成操作说明书。\n"
        "5. 戏剧任务与节奏设计审查员：先判断当前场景是赶时间、揭示、对峙、安抚、慢反应还是悬念落点；增强必须服务这个任务，而不是服务细节丰富度。\n"
        "6. 冲突强度与画面冲击审查员：检查增强后是否比原文更有冲突、更有可见压力和画面冲击；静态列队、泛泛忙乱、平铺直叙必须返修成有来源、有速度、有阻力的动作场面。\n"
        ...
        "允许极少量节奏型轻新增：仅限场景里自然会有、没有信息功能、可删掉也不改变主线的环境/生活阻碍，例如书本掉落、电话催促声、门铃/电梯提示、文件散开、杯子晃动；它只能体现快慢节奏或人物处境，不能变成新线索。\n"
        "增强版剧本必须写日常动作，不写抽象感受或刻意调度。禁止写“催促感、压迫感、紧张感、空气变静、压住急躁、把情绪压下去、把衣服放低”这类不可直接生成或不自然的句子；改成“手机又响了一声”“她蹲下来哄他”“她停了一秒没有说话”。\n"
        "轻新增还必须通过注意力/任务链合法性：它必须是角色当前正在处理、已经看见/听见且会自然影响当下目标的压力源。不要为了节奏加入角色不会注意的背景声音或远处动静，例如人在屋里专心给孩子穿衣时，普通走廊脚步声不应让她看门口；应优先使用闹钟、手机、时间、孩子、书包等当前任务链内元素。\n"
    ...
    def _review_user_prompt(candidate_output: str, round_index: int) -> str:
        return (
            ...
            "7. 戏剧任务与节奏设计：先判断场景任务，再看增强是否服务该任务。赶时间段可以用少量生活阻碍制造慌乱；慢反应段应使用停顿、沉默、动作中断和未完成动作，不要硬塞忙乱事件。\n"
            "8. 粗颗粒动作：穿衣、拿包、开门、坐下、递文件、看照片这类简单主动作必须保持一句话级别，不得拆成袖口、手指、脚步、角度等操作说明。若增强稿主要是在细化主动作，必须返修为节奏型增强。\n"
            "9. 日常动作语言：增强稿必须像剧本动作行，不像导演阐释。禁止“催促感压进来”“压住急躁”“空气变静”“把衣服放低”等抽象或刻意动作；应改为日常动作和声音：手机响、门铃响、她蹲下说话、她停了一秒没有接话。\n"
            "10. 注意力/任务链合法性：轻新增必须属于角色当前任务链，角色会自然注意并处理。禁止把普通背景声、远处动静、无关路人作为压力源硬塞进当前动作；例如屋内给孩子穿衣时，不用走廊脚步声打断，优先用闹钟再次响起、手机来电、时间显示、书包没拿好等。\n"
            "11. 轻新增合法性：允许场景自然存在的非关键环境/生活阻碍，但必须满足：不新增人物、不新增台词、不新增关键证据、不改变因果；可删掉也不影响主线。否则放入需用户确认。\n"
            "12. 冲突强度与画面冲击：判断增强后是否真的比原文更有戏剧压力和可见冲突；如果只是让人物静态列队、站着等待、泛泛忙乱，必须返修为符合原剧情和场景约束的动态动作。比如迎接新老板时，主管和秘书们静态站在门口不如从大门内急匆匆一路小跑出来、边整理文件边聚拢更有画面冲击；只要不改变主线，可以重构非主线场面动作、群体运动和压力来源。\n\n"
            "【必须输出的 YAML 字段】\n"
            ...
            "多维审查: 至少六项列表，分别包含 角色 / 通过 / 发现 / 处理；必须包含“戏剧任务与节奏设计审查员”和“冲突强度与画面冲击审查员”。\n"
            "硬错误: 列表；每条包含 类型 / 原句 / 问题 / 严重级别 / 必须修复 / 修复结果。没有硬错误写 无。\n"
            "评分: 包含 施事逻辑 / 道具连续性 / 人物动机 / 空间调度 / 注意力任务链 / 主线保护 / 可拍性 / 戏剧任务匹配 / 粗颗粒动作 / 冲突强度 / 画面冲击，每项 1-5 分；任一项低于 4 不得 PASS。\n"
    ...
    def _gate_verdict(output: str, reported_verdict: str) -> tuple[str, str]:
        ...
        for role in ("戏剧任务与节奏设计审查员", "冲突强度与画面冲击审查员"):
            if role not in review_panel_text:
                return "REPAIR_REQUIRED", f"missing_review_role:{role}"
...
def _director_showrunner_compact_prompts(
    *,
    ...
    system_prompt = (
        "你是短剧剧情增强导演。先判断场景任务和节奏类型，再做粗颗粒剧情增强。"
        "只在不改变主线、人物关系、剧情结果和原台词的前提下，把概括、静态或弱冲突的部分改成更有节奏压力的可拍事件。"
        "简单主动作不要细化成操作说明；允许极少量场景自然存在、非关键、可删不改主线的轻新增节奏阻碍。"
    ...
    user_prompt = (
        ...
        "剧情增强设计:\n"
        "  场景任务: ...\n"
        "  节奏类型: 快节奏/慢节奏/反应停顿/揭示/对峙\n"
        "  增强策略: ...\n"
        "增强版剧本: |\n"
        "  保留原台词原文；简单主动作保持粗颗粒，只补充服务节奏的少量事件。\n"
        "增强依据:\n"
        "  - 原文锚点: ...\n"
        "    增强方式: ...\n"
        "    权限级别: L1_节奏型轻增强\n"
...
def director_showrunner_node(state: DirectorState) -> DirectorState:
    ...
    showrunner_hint = (
        "剧情增强 戏剧任务 节奏型轻增强 粗颗粒动作 快慢节奏 冲突强化 "
        "时间压力 声音压力 非关键生活阻碍 主线保护 禁止新增台词 禁止改主线 "
        f"画幅 {state.get('aspect_ratio', '16:9')}"
    )
    system_prompt, retrieval_meta = build_system_prompt(
        "你是 AI 短剧流水线里的剧情冲突增强导演。\n"
        "你的职责是在不改变主线剧情的前提下，先判断场景戏剧任务和节奏类型，再把原剧本里偏弱、偏概括、偏静态的部分增强成更有节奏设计的可拍内容。\n"
        "你不做拆片、不做具体镜头设计、不输出 shot 建议；你只交付增强版剧本和增强依据。\n"
        "你的增强必须是清晰的粗颗粒剧情设计，不是文学化润色，也不是动作操作说明；少用形容和比喻，只写观众能看见、演员能执行、模型容易生成的日常动作和声音。\n"
        "禁止输出状态合同、入场状态、出场状态、道具状态变化、禁止连续性等制作合同块；这些只可内化为判断，不能写进增强版剧本。\n"
        "简单主动作必须保持粗颗粒：穿衣、拿包、开门、坐下、递文件、看照片等通常一句话即可；不要细化到袖口、手指、脚步路径、身体角度。\n"
        "允许增强 L1 节奏型轻事件和笼统相对互动：时间压力、声音压力、场景自然存在的非关键生活阻碍、靠近、停住、避让、拦住、转身等可见动作。\n"
        "允许极少量原剧本未写但符合场景常识的轻新增，只能是非关键、无信息功能、可删不改主线的节奏阻碍，例如赶时间时闹钟又响、手机又响了一声、书包没拿好、慢节奏时人物停了一秒没有说话。它不能成为新线索、新证据、新人物、新台词或新反转。\n"
        "所有轻新增必须在角色当前注意力和任务链内：角色正在处理什么，就只能加会自然影响这件事的压力源。不要加角色不会注意或不会处理的背景声音，例如屋里给孩子穿衣时不要写走廊脚步声让她看门口；应使用闹钟、手机、时间、孩子、书包等当前已存在或强相关元素。\n"
        "禁止用抽象名词替代动作：不要写“催促感、压迫感、紧张感、空气变静、急躁、情绪压下去”。也不要写不自然的摆拍动作，例如“把衣服放低”。应写成日常动作：她蹲下来哄孩子、她停了一秒、她没有接话、手机又响了一声。\n"
        ...
        "关键道具处理必须保持因果清晰：原剧本关键道具或台词暗示道具必须写清去向；轻新增环境道具只能当作节奏阻碍，不得承担信息或因果功能。\n"
        "手机/电话尤其要谨慎：如果原台词暗示正在通话，可以写乔熙拿着或放下手机；通话结束后必须写清手机去向，不能让手机持续占手却又同时完成双手动作。\n"
        "输出前必须进行逻辑审查：动作施事必须合理，无生命道具不能像有意志一样行动；群体调度必须符合人物当下目标和场景前置状态；增强是否服务戏剧任务而非细节堆砌。\n"
    ...
    user_prompt = (
        ...
        "【必须输出的 YAML 字段】\n"
        "剧情增强设计: 包含 场景任务 / 人物目标 / 阻碍设计 / 节奏类型 / 粗颗粒原则；不要写镜头方案。\n"
        ...
        "【决策边界】\n"
        "1. 先写清场景任务，再增强。赶时间段增强具体的“顾不上”：闹钟又响、手机响了没接、东西掉了没捡、书包没拿好；慢节奏段增强可见停顿：人物停了一秒、没有接话、没有继续动作；揭示段增强信息命中与反应刹车；对峙段增强阻挡、沉默、让路或不让路。\n"
        "2. 可以把“忙乱、急匆匆、气氛紧张、愣住、等待、列队”等概括词改成服务节奏的少量可见事件，但不要把核心动作拆细。例如“给孩子穿衣服”不要写怎么套袖子，可以写旁边书掉了她顾不上捡，用来体现慌乱。\n"
        "3. 可以把静态说明改成笼统可见动作，例如已有角色出现、停住、靠近、避让、拦住、转身、递出或收回道具；也可以把非主线的静态场面重构成动态压力场面，例如让主管和秘书们从合理入口急促小跑聚拢，而不是静态站在门口列队。不要写具体站位、方位、距离或路线。\n"
        "4. 可以使用原剧本已有道具和环境强化阻力，例如闹钟、电话、水杯、书包、咖啡、公司大门、车辆声音。也允许极少量场景自然存在且在角色注意力/任务链内的非关键轻新增，例如书本、本子、提示声、散开的文件；它们只能制造节奏，不得传递新剧情信息。\n"
        "5. 每两句原台词之间最多补 1-2 个核心节拍；优先写日常动作或声音，不写情绪散文、抽象感受，不把简单主动作拆成过多微动作。\n"
        "6. 手机/电话规则：只有原文台词、动作或上下文明示通话时才可使用手机；如果写手机夹在肩上、握在手里或放在一旁，后续动作必须符合单手/双手可执行逻辑。\n"
        "7. 增强版剧本只写场景标题、人物、动作、原台词和必要转场；不要写“状态合同/入场状态/出场状态/道具状态变化/禁止连续性/特写/音效”等合同式或镜头式小标题，除非原文已有。\n"
        "8. 只遵守上方简表里的物理空间硬约束：入口、通道、固定物体和可见道具不能改乱；不要推导或锁定人物站位、行动路径、运动轨迹。\n"
        "9. 如果场景预分析包含“关键道具缺口”或“可补道具约束”，代表该道具来自原剧本/台词但参考图里缺失或不可确认；可以在合理可见空间内补足它的起始位置和去向，但不能新增剧本外关键道具。\n"
        "10. 没有明确空间信息时，使用“靠近、停下、退开、挡住、绕开、转身看向”等笼统关系词，不写“左侧/右侧/北侧/几米/从A点到B点”等精确调度。\n"
        "11. 不得改变主线剧情：人物关系、公司易主、新老板到达、前夫揭示等核心事实不能变。\n"
        "12. 不得新增台词；原台词必须原样保留。\n"
        "13. 除原剧本台词或专有名词外，不要输出英文标签、英文小标题或英文字段名。\n"
        "14. 输出前做一次硬逻辑自检：施事是否真实可动，人物是否有合理动机，新增事件是否在角色注意力/任务链内，群体是否从合理位置进入/聚拢，道具是否占手冲突，空间前置状态是否成立，简单主动作是否被过度细化，是否出现抽象感受词或不自然摆拍动作。\n"
    )
