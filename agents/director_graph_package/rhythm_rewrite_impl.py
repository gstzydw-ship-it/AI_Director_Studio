...
def _rhythm_supervisor_role_prompt() -> str:
    return (
        ...
        "- 急促动作/短位移/冲入/小跑/开门入场/上下车/电梯进出这类短动作，默认建议时长 2-5秒；只有同时包含完整发言、信息揭示或明确反应落点时，才可放宽到 4-6秒。\n"
        "- 快节奏戏剧任务优先使用 2-6秒短段；11-15秒只允许给多事件紧凑段或长对白段，不能给单一急促动作。普通段也只能写 13-15秒，不得写 18-22秒。\n\n"
        "硬边界与完整性：\n"
...
def _rhythm_supervisor_user_prompt(state: DirectorState) -> str:
    ...
    return (
        ...
        "12. 急促动作、短位移、冲入、小跑聚拢、开门入场、上下车或电梯进出，如果没有完整发言/信息揭示/反应落点，建议时长不得超过 5秒；不得把它写成 8-10秒 或让人物慢慢完成。\n"
        "13. 输出必须以完整的 风险提醒 字段结束；不得停在“不能包含的后”“建议时”等半截字段或半截句子。\n"
    )
