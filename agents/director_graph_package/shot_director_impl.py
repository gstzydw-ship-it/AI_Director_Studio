...
def _shot_director_coverage_contract_prompt() -> str:
    return (
        ...
        "- template_plan：只写 shots 镜头表；每个 shot 必须有 shot_id、duration、coverage_role、task、subject、shot、action、dialogue、must_carry、cut_reason、cut_point、continuity、tailframe_role。\n"
        "- 每个 shot 还必须补齐 Seedance 白名单合同字段：template_id、template_level、reference_need、model_complexity_score、tail_state；template_level 只能是 W1/W2/R1/X/candidate，但 X/candidate 只能用于打回说明，不得进入 prompt_compiler。\n"
        ...
        "template_plan.shots 是下游 prompt_compiler 的唯一主镜头表；不得再输出旧 v1/v2 的 fragment_task/rhythm/shots 中文自由镜头表、main_shots、sub_shots 或 construction sheet。\n\n"
        "【每个 shot 需要补齐的职责字段】\n"
        "- coverage_role：这镜负责什么覆盖任务，例如 relation_setup、dialogue_hold、reaction_landing、prop_insert_proxy、tail_bridge。\n"
        "- cut_reason：为什么必须在这里切，必须绑定动作顶点前、台词断点、信息看清、反应出现或尾帧完成。\n"
        "- tailframe_role：这一镜尾帧怎样交给下一镜或下一片段。\n"
        "这些字段是给提示词编译师的施工依据，不能写成空泛形容词；最终必须保留 v3 英文字段名，字段内容用自然中文。\n"
    )
...
def _shot_director_layout_rule_block(aspect_ratio: str) -> str:
    return (
        ...
        "输出只允许是施工 YAML，不要解释。\n\n"
        "【必须输出】\n"
        "- fragment_id、schema_version: shot_director_coverage_v3\n"
        "- coverage_plan：dramatic_task、rhythm_intent、space_contract、shot_budget、required_beats\n"
        "- template_plan.shots：阶段一只搭可继承的主镜头覆盖链，字段使用 v3 英文键\n"
        "- guard_result：status、repairs、final_shots\n\n"
        "【阶段一边界】\n"
        "1. 只搭主镜头骨架，不输出旧版中文自由镜头表，不细拆 sub_shots。\n"
...
def _shot_director_blocking_rule_block(aspect_ratio: str) -> str:
    return (
        ...
        "【必须输出】\n"
        "- 保留一号的 fragment_id、coverage_plan、template_plan.shots 和 guard_result。\n"
        "- 补齐 template_plan.shots 中的：duration、coverage_role、task、subject、shot、action、dialogue、must_carry、cut_reason、cut_point、continuity、tailframe_role、template_id、template_level、model_complexity_score、reference_need、tail_state；其中 shot 字段只写主体景别、视角/观看位置和必要运动，动作表情必须写进 action。\n"
        "- 不再输出 sub_shots；必要短插入必须作为 W1/W2/R1 模板 shot 写入 template_plan.shots，并给出 cut_reason/cut_point/tail_state。\n"
...
def _shot_director_guard_stage_rule_block(aspect_ratio: str) -> str:
    return (
        ...
        "【最终 YAML 必须使用 v3 英文字段】\n"
        "- fragment_id、schema_version: shot_director_coverage_v3、coverage_plan、template_plan、guard_result、tail_state\n"
        "- template_plan.shots 每个 shot 必须含 shot_id、duration、coverage_role、task、subject、shot、action、dialogue、must_carry、cut_reason、cut_point、continuity、tailframe_role、template_id、template_level、model_complexity_score、reference_need、tail_state\n"
        "- 禁止输出旧版片段任务/节奏/空间连续性总控/镜头列表作为最终主结构；旧字段只可作为已废弃输入被改写成 v3。\n\n"
        "【空间连续性总控】\n"
        "每个片段在镜头列表前必须先写一句片段级连续性总控：本片段是一段什么戏剧任务，哪些人物始终处在同一空间内；"
        "单人镜只表示镜头主体变化，不代表其他在场人物离开；每镜继承上一镜尾帧、道具状态、视线方向和同侧轴线。\n\n"
        "【最终镜头表达】\n"
        "shot 字段只写最终可生成的视角表达：主体景别 + 视角/观看位置 + 必要运动/前景关系；不要写戏剧判断、人物动作、台词或表情。例如“乔熙胸部以上中近景，从商北琛肩后看向她”。\n"
        f"{_SHOT_VIEWPOINT_TRANSLATION_HINT}\n"
    ...
    *,
    ...
    if existing_final:
        ...
        user_prompt = (
            ...
            "  coverage_plan:\n"
            "    dramatic_task: 本片段的剧情施工任务\n"
            "    rhythm_intent: 服从节奏总控的节奏指令\n"
            "    space_contract: 同一空间、同一人物组、每一镜继承上一镜尾帧和道具状态\n"
            "    shot_budget: 1-4 shots unless information/reaction requires more\n"
            "    required_beats: [relation_setup, event_atom, tail_bridge]\n"
            "  template_plan:\n"
            "    shots:\n"
            "      - shot_id: F01-S01\n"
            "        duration: 0-2秒\n"
            "        coverage_role: relation_setup/dialogue_hold/reaction_landing/prop_insert_proxy/tail_bridge\n"
            "        task: 建立关系/承载对白/动作推进/信息揭示/反应落点/尾帧承接\n"
            "        subject: 人物名/双人关系/剧本已有道具\n"
            "        shot: 只写摄影选择，例如双人半身关系镜，平视，固定视角\n"
            "        action: 起始状态 -> 动作变化 -> 视线/表情/身体反应 -> 镜尾状态\n"
            "        dialogue: 原剧本台词/画外音或 ~\n"
            "        must_carry: 这个镜头必须看清的信息、道具状态、人物距离变化或表演落点\n"
            "        cut_reason: 为什么切到下一镜\n"
            "        cut_point: 动作顶点/台词断点/信息看清/反应出现/状态完成/尾帧\n"
            "        continuity: 动作、道具、人物左右关系、轴线或尾帧状态如何继承\n"
            "        tailframe_role: setup/bridge/final_tail\n"
            "        template_id: COV-SD20-W1-RELATION-HOLD\n"
            "        template_level: W1\n"
            "        model_complexity_score: 1\n"
            "        reference_need: identity_reference + scene_reference\n"
            "        tail_state: 人物位置、视线/注意力、道具状态和未解决问题\n"
            "  guard_result:\n"
            "    status: pass\n"
            "    repairs: []\n"
            "    final_shots: [F01-S01]\n"
            "  tail_state: 可被 storyboard_designer 和 prompt_compiler 继承的尾帧状态\n\n"
            "【关键要求】\n"
            "1. 必须覆盖拆片方案的所有片段编号。\n"
            ...
            "7. 每个镜头的时间段必须连续，前后衔接。\n"
            "8. 必须输出 coverage_plan、template_plan、guard_result；template_plan.shots 是下游唯一主镜头表。\n\n"
            "9. 每个片段的空间连续性必须写入 coverage_plan.space_contract，不再输出旧版【空间连续性总控】主字段。\n\n"
            ...
            f"{rule_block}"
            "请只输出完整 YAML v3 镜头方案，禁止回退旧版中文自由镜头表。"
        )
        ...
        if _shot_stage_should_split(expected_segments, downstream_context):
            ...
            def build_fragment_prompt(fragment_id: str, fragment_context: str, _fragment_contract: str) -> str:
                return (
                    ...
                    "- guard_result\n"
                    "- template_plan.shots\n"
                    "- shot_id / duration / coverage_role / task / subject / shot / action / dialogue / must_carry / cut_reason / cut_point / continuity / tailframe_role\n"
                    "- template_id / template_level / model_complexity_score / reference_need / tail_state\n\n"
                    "【规则】\n"
                    "1. 只输出这个片段的 YAML，并且第一行必须是 '- fragment_id:'。\n"
                    "2. shot_id 保持稳定，例如 F01-S01、F01-S02。\n"
                    "3. 时长必须是片段内连续时间段，例如 0-2秒、2-5秒。\n"
                    "4. 长对白需要听者反应覆盖和具体切镜点。\n"
                    "5. 不新增剧本外元素。\n"
                    "6. 必须输出 v3 英文字段名，禁止回退旧版中文字段。\n"
                    "只输出 YAML。"
                )
    ...
    if final_issues and split_output_complete:
        ...
        repair_prompt = (
            ...
            "2. 每个返修片段必须有：fragment_id、schema_version: shot_director_coverage_v3、coverage_plan、template_plan、guard_result、tail_state。\n"
            "3. template_plan.shots 每个 shot 必须有字段：shot_id、duration、coverage_role、task、subject、shot、action、dialogue、must_carry、cut_reason、cut_point、continuity、tailframe_role、template_id、template_level、model_complexity_score、reference_need、tail_state。\n"
            "4. 切镜点必须绑定动作顶点、台词断点、信息看清、反应出现或尾帧状态。\n"
            "5. 长台词必须插入听者反应镜头；不新增剧本外元素。\n\n"
            "6. 禁止回退旧版中文自由镜头表；最终字段名必须使用 v3 英文合同键，字段内容用中文。\n\n"
...
def _run_shot_director_three_stage_impl(
    *,
    ...
    layout_profile = _shot_director_stage_profile(
        ...
        extra_constraints=[
            "layout 阶段只搭 v3 template_plan.shots 覆盖链，不输出 reaction_coverage 或 sub_shots",
    ...
    layout_user_prompt = (
        ...
        "  schema_version: shot_director_coverage_v3\n"
        "  coverage_plan:\n"
        "    dramatic_task: 本片段的戏剧施工任务\n"
        "    rhythm_intent: 上游情绪曲线/节奏意图/必须保留落点/可压缩弱拍/尾帧承接\n"
        "    space_contract: 场景锚点、人物起点、主要视线/动作轴线和禁止越轴说明\n"
        "    shot_budget: 1-4 shots\n"
        "    required_beats: [relation_setup, event_atom, tail_bridge]\n"
        "  template_plan:\n"
        "    shots:\n"
        "      - shot_id: F01-S01\n"
        "        coverage_role: relation_setup/action_start/reaction_landing/tail_bridge\n"
        "        task: 建立关系/行动起势/受击承接/关系复位/尾帧承接\n"
        "        subject: 人物名、双人关系或剧本已有道具\n"
        "        shot: 主镜头骨架表达，景别 + 简洁视角 + 核心可见关系\n"
        "        cut_reason: 为什么需要切到这里\n"
        "        tailframe_role: setup/bridge/final_tail\n"
        "  guard_result:\n"
        "    status: draft\n"
        "    repairs: []\n"
