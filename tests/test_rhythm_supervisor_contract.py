...
def test_rhythm_supervisor_outputs_time_and_segment_contract(monkeypatch):
    ...
    assert "急促动作/短位移/冲入/小跑/开门入场" in captured["system_prompt"]
    assert "默认建议时长 2-5秒" in captured["system_prompt"]
    ...
    assert "目标不得超过 5秒" in captured["user_prompt"] or "建议时长不得超过 5秒" in captured["user_prompt"]
